...
static inline void shift_window(int t, kdq_t(int) *w, int T, int W, int *L, int *rw, int *rv,
                                int *cw, int *cv) {
    int s;
...
static inline void save_masked_regions(uint64_v *res, perf_intv_v *P, int start) {
    ...
    for (i = P->n - 1; i >= 0 && P->a[i].start <
                                 start; --i); // remove perfect intervals that have falled out of the window
    P->n = i + 1;
}

static void find_perfect(perf_intv_v *P, const kdq_t(int) *w, int T, int start, int L, int rv,
                         const int *cv) {
    ...
    for (i = l = t = 0; i <= l_seq; ++i) {
        ...
        if (b < 4) { // an A/C/G/T base
            ...
            if (l >= SD_WLEN) { // we have seen a word
                start = (l - W > 0 ? l - W : 0) +
                        (i + 1 - l); // set the start of the current window
                save_masked_regions(&buf->res, &buf->P,
            ...
            while (buf->P.n)
                save_masked_regions(&buf->res, &buf->P,
                                    start++); // clear up unsaved perfect intervals
            l = t = 0;
        }
    }
    *n = buf->res.n;
