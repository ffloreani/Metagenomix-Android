...
void mm_sketch(const char *str, int len, int w, int k, uint32_t rid, mm128_v *p) {
    ...
    for (i = l = buf_pos = min_pos = 0; i < len; ++i) {
        ...
        if (c < 4) { // not an ambiguous base
            ...
            if (++l >= k)
                info.x = hash64(kmer[z], mask), info.y = (uint64_t) rid << 32 | (uint32_t) i << 1 |
                                                         z;
        } else l = 0;
        buf[buf_pos] = info; // need to do this here as appropriate buf_pos and buf[buf_pos] are needed below
        if (l == w + k -
                 1) { // special case for the first window - because identical k-mers are not stored yet
        ...
        if (info.x <= min.x) { // a new minimum; then write the old min
            ...
            for (j = buf_pos + 1, min.x = UINT64_MAX;
                 j < w; ++j) // the two loops are necessary when there are identical k-mers
                if (min.x >= buf[j].x)
